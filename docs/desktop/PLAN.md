...
| A6 | `bridgeId` persistence = a small file-backed storage **inside `auth/`** (no Dao) — **implemented as `BridgeIdStorage`** | one string; no DB/migration; keeps it within the self-contained auth subsystem so auth code doesn't depend back on top-level `repositories/`. The pre-v1.4 `token.json` adoption seam was retired after the compatibility baseline advanced. |
