...
| GUI→helper send paths beyond `token_response` | OPEN — deferred by PR 2.5 | TBD | PR 2.5's dispatcher answers `token_request` only. The `prompt_response` answer sender lands with its consumer (PR 2.7 "Take over" / PR 2.9 tray actions — a cubit cannot call the Layer-4 dispatcher, so the sender seam is designed at that PR's plan review); `unregister_and_exit` send is PR 2.13; `token_update` push is an optimization (the supervised bridge pulls with forceRefresh on demand — PR 1.4/1.5) owned by whichever PR first needs it. |
