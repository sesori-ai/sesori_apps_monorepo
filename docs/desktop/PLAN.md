...
- **Next up:** Phase 2 — PR 2.6 `BridgeProcessService` (spawn/kill/path + expected-stop boundary + helper log capture) — **paused until the open stacked PRs merge (user decision)**. **Phase 1's MT-1 manual checkpoint (user-run) remains OPEN** — run it before calling Phase 1 done.
...
| GUI→helper send paths beyond `token_response` | OPEN — deferred by PR 2.5 | TBD | PR 2.5's dispatcher answers `token_request` only. Three deferred sub-items, each with an owning PR whose Acceptance carries the note: `prompt_response` answer sender → PR 2.7/2.9 (whichever lands the first prompt answer; a cubit cannot call the Layer-4 dispatcher, so the sender seam is designed at that PR's plan review); `unregister_and_exit` send → PR 2.13; `token_update` push → PR 2.15 (its E2E goal already exercises token push/pull; the bridge is correct pull-only meanwhile — PR 1.4/1.5). The row stays until ALL three are owned/landed; strike sub-items individually. |
| Client `AuthManager` never emits `unauthenticated` on a server-rejected refresh | OPEN | TBD | Pre-existing, platform-wide: `_doRefreshAndPersist` returns null on ANY failure (offline and revoked look identical) and no auth-state emission happens, so a revoked-but-locally-unexpired session is never reactively signed out — mobile routes to projects on `hasLocallyValidSession` alone, and the desktop gate keeps a provisional `signedIn` on token-only sessions (comment in `AuthGateCubit` points here). Fix belongs in `module_auth`: distinguish server-rejection (4xx from `/auth/refresh`) from transport failure and emit `unauthenticated` on rejection. Surfaced by PR 2.3 review; manual sign-out is the workaround. |
