...
| `PluginStateStorage.legacySharedRuntime` on the OpenCode and Codex descriptors | Released installs stored managed binaries and ownership records beneath the shared runtime root. Moving either descriptor would strand offline binaries and crash-recovery state. | Move a descriptor to isolated storage only after an atomic migration for its runtime subdirectory and ownership file has shipped, and the minimum supported bridge is newer than that migration. | Both storage layouts predate parallel startup; no migration release or minimum-supported bridge is scheduled. |
