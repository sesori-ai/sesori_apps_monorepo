...
- Permission dialogs use the shared floating Prego action sheet in both themes:
  an action-specific heading when the wire request supplies authoritative
  ...
  remain available (an identical command is displayed only once), then stacked **Allow**, optional
  **Always approve**, and **Don’t allow** actions. **Allow** means once only.
  ...
  it without a reply. Inline text/voice instructions are never offered. Reject
