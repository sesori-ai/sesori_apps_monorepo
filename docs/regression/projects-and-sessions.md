...
- The retired `--import-plugin` CLI option remains accepted only to print one
  user-facing warning. It does not validate the supplied harness identifier or
  start any import, and the warning directs the user to pull to refresh or
  Settings > Harnesses in the Sesori app.
...
- The retired CLI import option starts or validates an import, exits without its
  deprecation warning, or suggests a workflow outside the Sesori app.
...
- Tests: `bridge/app/test/bridge/runtime/run_command_catalog_import_test.dart`,
  `bridge/app/test/bridge/routing/catalog_read_handlers_test.dart`,
