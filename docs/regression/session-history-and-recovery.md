...
- Claude's CLI-authored API-failure assistant frame and its terminal result
  ...
  without duplicates; nested sub-agents replay under the root. A command Claude
  queued mid-turn (a follow-up, a peer message or a task outcome, persisted as
  a `queued_command` attachment) replays where the model received it with the
  live frame's id, sender and parts: follow-ups as user messages, peers and
  unabsorbed task outcomes as Automation, and a known task's outcome folded into
  its tile, so reload and re-import keep it in the same turn as live.
- A `user` frame Claude's CLI generated itself never renders as a user turn,
  live or replayed. The CLI flags it `isSynthetic` on the stream and `isMeta` in
  the transcript; both spellings mean the same thing and are normalized at the
  parse boundary, so an injected skill body, an image caption, a continuation
  nudge and sub-agent plumbing are dropped instead of rendering the whole text
  as a user bubble. Invoking a skill therefore shows only its `Skill / <name>`
  step row. Sesori's own peer injections carry the same flag and stay visible as
  Automation, as do tool results, task-notification outcomes and the summary
  that follows a compaction boundary. Material failure signals: a skill body,
  image caption or automation preamble appearing as a user bubble during a live
  turn, a peer message disappearing, or live and replay disagreeing about which
  user turns are visible.
...
- A Claude API failure appears once as ordinary assistant text and again as an
  ...
  transcript duplicates its parts after reload. A mid-turn Claude follow-up,
