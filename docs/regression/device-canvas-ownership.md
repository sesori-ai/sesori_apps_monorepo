...
| Antigravity | Disabled by the shared ACP default-deny policy. | Device Canvas HTTP MCP invocation not verified; no new runtime probe during rebase. |
...
The versions above record integration/protocol evidence. The rebased implementation
retains main's newer managed targets (OpenCode 1.18.30, Codex 0.154.0, Claude
2.1.269, Pi 0.85.1, Cursor 2026.09.10-fd3934a, OMP 18.1.19, Copilot 1.0.83)
without claiming a fresh live model/device run for those targets. Bridge database
v16 adds claims and revisions while preserving main's v1–v15 schema history,
new-session defaults, and options cache.
