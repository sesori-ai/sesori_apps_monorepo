...
- Singular starts only on Android/iOS, in the same eligible release-build population, with required credentials
  injected outside Git. Startup and event reporting are best-effort and must not block the app. The configuration
  limits advertising identifiers and partner data sharing, removes Android advertising-ID permissions, and sets no
  custom user ID, custom events, deep-link handler, or uninstall token. The Basic Usage Analytics preference does not
  claim to control this separate attribution scope.
...
- Singular starts in debug/profile/unsupported builds or during the gated Android crawl window; requests an
  advertising-ID permission, sets a custom user identity, sends custom events, attaches authentication-event
  attributes, broadens partner sharing, omits login after successful interactive authentication, reports registration
...
under `client/module_core/lib/src/foundation/models/product_analytics/` and
`client/module_core/lib/src/foundation/models/attribution/`; Singular startup and event delivery live under
