...
may start Singular for install/session attribution when both build credentials are configured. Desktop uses a no-op
sink, the bridge is excluded, and the warehouse is external.
...
- Singular starts only on Android/iOS, in the same eligible release-build population, and with a complete SDK key/secret
  pair injected outside Git. Startup is best-effort and must not block the app. The initial configuration limits
...
- Singular starts with a partial credential pair, in debug/profile/unsupported builds, or during the gated Android crawl
  window; requests an advertising-ID permission, sets a custom user identity, sends custom events, or broadens partner
  sharing.
...
- Singular remains dormant until both repository secrets are configured. Its bundled iOS privacy manifest still
  declares linked device-ID tracking and advertising/analytics purposes, so store-metadata review applies even while
  runtime startup is dormant. Vendor-side retention, deletion, and campaign setup are external release gates and
