...
- The interaction service selects only advertised `allow_once` or `reject_once` choices without `agy.security.warning`
  metadata. Persistent or unknown kinds are excluded. A normal permission requires exactly one usable allow-once
...
- Ordinary approval returns the exact advertised allow-once ID. Reject returns the advertised reject-once ID when
  ...
  snapshots do not advertise always-allow. No missing option ID is synthesized.
...
- Malformed-request recovery remains observable through a safe typed exception and local log with the original stack.
  Decoder causes are retained for inspection but are not implicitly rendered, because they may include prompt text.
...
- `antigravity_interaction_service_test.dart` uses real ACP response encoding with a fake process and ten focused tests:
  ...
  unknown methods; cancellation/disposal and stream detachment; and independent connection dispatch.
- Owning package analysis and those ten tests passed at the initial code checkpoint. The updated predecessor changes
  model-options behavior only; this slice does not activate or verify shared ACP registry wiring or live Google tools.
