...
- Mobile and desktop route root, non-archived session detail into the same shared
  ...
  lines use native selection/context-menu behavior, including inside the
  desktop cockpit's wide right pane.
...
- Line counts disagree with content, especially untracked or deletion-only, or
  diff source text cannot be selected through its native context menu.
