...
- Private upgrade qualification must consume two retained successful native packaging
  ...
  press its process-owned status item, accept only a new process-owned menu anchored to
  that item's frame, invoke Quit only inside that menu, reject relaunch/orphans,
