...
- Private upgrade qualification must consume two retained successful native packaging
  ...
  record the process-owned status-item frame, press it, type-select Quit and invoke only
  the focused exact PID/title item in a menu anchored to that frame. Reject relaunch/orphans,
