...
- A blocked harness never hides history: a cold blocked chat loads and renders the
  transcript from the bridge database with the notice in the composer's place, and a
  live block, reload race or metadata-refresh failure preserves the rendered
...
|---|---|
| L1 Smoke | Automated shared presentation and desktop shell coverage: representative selectable transcript content renders through the shared session-detail view, the desktop session route is present, and desktop renders the text-first composer and declared attachment/diff actions without resolving voice capture. A blocked fixture shows its reason in the composer's place, with the transcript still rendered and no pending-input banners; setup-relevant fixtures retain Harness Settings, authentication-required fixtures alone additionally show Recheck, and local queue cancellation remains available. Live plugin, representative: a prompt streams assistant output and returns the session to idle. |
...
- A cold block hides the stored transcript behind a full-screen notice, renders an
  empty transcript as if history loaded, or omits its reason/settings path. A live
  block blanks already rendered content or drops buffered events.
- Controls return before management and session prerequisites refresh, require a
