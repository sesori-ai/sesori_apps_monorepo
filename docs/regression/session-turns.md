...
- The bridge owns accepted-but-not-yet-visible prompts. An entry appears in the
  ...
  is refused because the prompt is then governed by Stop. Dispatched entries
  remain visible with a Sending indicator, not a trash action. Older public
  bridges without dispatch ownership retain best-effort cancellation.
  A refused/not-found cancellation surfaces a notice; only success removes a
  row as cancelled. Existing queue events and reconnect snapshots update the
  display. Transport failure retains the row rather than implying cancellation.
  Repeated taps on one prompt share the pending cancellation attempt without
  generating a false refusal, and a failed attempt can be retried. Aborting the
...
- Pending prompts sit inside every composer layout above its input/actions,
  ...
  single-line ellipsized preview and, while bridge-pending, a trailing trash action
  that cancels only that prompt. Newlines flatten to spaces; slash-command names and attachment
  counts remain visible in the preview instead of Markdown or image thumbnails.
  Longer queues show three full rows plus half the next row as a scroll cue,
  scrolling independently of the transcript. Shorter queues fit their contents;
  larger accessibility text increases row height rather than clipping. Cancelling does
...
- A send or cancel succeeds against an archived session, an aborted turn
  ...
  the pending list escapes the composer, grows beyond three-and-a-half rows without scrolling,
  or a trash action cancels another prompt or clears the draft. A prompt reported
  as dispatched still offers trash, an older public bridge loses best-effort
  cancellation, repeated taps produce a false refusal, or a refused cancellation
  hides work as successfully cancelled before it later appears in the transcript. Delivered message
