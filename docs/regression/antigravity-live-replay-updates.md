...
- Command/CWD labels are trimmed and bounded to 8,000 characters. Command becomes the display title; execute kind is
  inferred only when no kind is advertised. Output aliases become standard stdout, retaining the numeric exit code.
...
- When native output duplicates standard text content, display it once. Preserve original non-text content entries,
...
- `antigravity_protocol_mapper_updates_test.dart`: 15 tests cover exact aliases, identity, live/replay equality,
