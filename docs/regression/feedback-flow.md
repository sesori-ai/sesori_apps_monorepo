...
- Product analytics (account-linked, under the Basic Usage Analytics
  ...
  - `feedback_prompt_answered`, once per sheet after it has closed, with
    `answer`: `love_review_requested` (**Leave a review**, or the iOS automatic
    StoreKit request), `love_no_review` (**Yes**, then **Not now**, the close
    button or a swipe down), `could_be_better` (sent or not), or `dismissed`.
