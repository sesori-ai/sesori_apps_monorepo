...
- A correlated management command acknowledgment remains successful when an intervening
  same-bridge refresh supersedes only its snapshot. The client discards that response's
  snapshot and reconciles through GET, retaining refresh failures separately. Success is
  never inferred from active, idle, or enabled metadata. Response loss, unprovable responses,
  and connection/bridge identity changes remain uncertain; reconciliation cannot revive
  a fenced acknowledgment or publish a stale response.
...
- A correlated enable/restart acknowledgment becoming uncertain solely because a refresh
  published first, a superseded response overwriting newer metadata, or refreshed active/idle
  state being used to manufacture success after response loss or identity invalidation.
