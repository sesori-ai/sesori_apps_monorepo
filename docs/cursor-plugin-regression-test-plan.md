...
| SES-06 | Archive the session, show archived sessions, read it, and attempt an unarchive. | Archiving is permanent: history remains readable, the session becomes read-only, and an unarchive request returns the typed `archived_read_only` rejection without changing state. | Session list/messages/UI and expected 409 route response. Continue. |
