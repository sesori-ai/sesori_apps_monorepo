...
/// A circular "glass" icon button matching the Figma `pregoButtonsIconGlass`
/// component.
///
...
/// split used by `PregoGlassBackground`.
///
/// In both cases a `buttonGlassPrimaryHover` overlay is applied on hover/press
/// via [PregoTappable]. There is no border or shadow — only the glass/fill.
///
...
/// ```
///
/// Limitation (v1): unlike [PregoButtonsSolid], this control does not render a
/// keyboard focus ring on web/desktop (it relies solely on [PregoTappable],
...
class PregoButtonsIconGlass extends StatelessWidget {
  const PregoButtonsIconGlass({
    ...
    this.size = PregoButtonsIconGlassSize.md,
    this.iconSize,
    this.iconColor,
    this.semanticLabel,
  });
  ...
  double get _diameter => switch (size) {
    PregoButtonsIconGlassSize.md => 48.0,
    PregoButtonsIconGlassSize.lg => 64.0,
  };

  double get _defaultIconSize => switch (size) {
    PregoButtonsIconGlassSize.md => 24.0,
    PregoButtonsIconGlassSize.lg => 24.0,
  };
  ...
  Widget build(BuildContext context) {
    ...
    final button = PregoTappable.stateAware(
      onTap: onPressed,
