...
///   - `prego_color_primitives.g.dart`
///   - `prego_colors.g.dart`
///   - `prego_spacing_primitives.g.dart`
///   - `prego_spacing.g.dart`
///   - `prego_radius.g.dart`
///   - `prego_widths.g.dart`
///
...
/// - **Color Primitives** (mode-invariant base palette): `PregoColorPrimitives`
/// - **Semantic colors** (Dark/Light modes referencing primitives): `PregoColors`
/// - **Spacing Primitives** (numeric spacing scale): `PregoSpacingPrimitives`
/// - **Semantic spacing** (named tokens referencing primitives): `PregoSpacing`
/// - **Radius** (border radius tokens): `PregoRadius`
/// - **Widths** (width tokens referencing primitives): `PregoWidths`
///
/// Usage:
/// ```bash
...
Reads scripts/figma_tokens/figma_tokens.json and regenerates:
  lib/.../prego_color_primitives.g.dart
  lib/.../prego_colors.g.dart
  lib/.../prego_spacing_primitives.g.dart
  lib/.../prego_spacing.g.dart
  lib/.../prego_radius.g.dart
  lib/.../prego_widths.g.dart
""");
}
...
void _runGenerate() {
  ...
  final spacingPrimitives = (processed["spacingPrimitives"] as List<dynamic>?)?.cast<Map<String, dynamic>>() ?? [];
...
void _generatePrimitivesFile(List<Map<String, dynamic>> primitives) {
  ...
  buf.writeln("/// Semantic tokens in [PregoColorsDark] and [PregoColorsLight] reference");
  ...
  buf.writeln("/// `context.prego.colors.*` instead.");
  buf.writeln("abstract final class PregoColorPrimitives {");
...
void _generateColorsFile(
  ...
  // --- PregoColorsDark ---
  buf.writeln();
  _writeStaticColorClass(buf, "PregoColorsDark", "Dark", colors, "dark", primitiveDartNames, semanticDartNames);

  // --- PregoColorsLight ---
  buf.writeln();
  _writeStaticColorClass(buf, "PregoColorsLight", "Light", colors, "light", primitiveDartNames, semanticDartNames);
...
void _writeStaticColorClass(
  ...
  buf.writeln("/// Colors reference [PregoColorPrimitives] where Figma uses an alias,");
  ...
  for (final color in colors) {
    ...
    if (type == "primitive") {
      ...
      if (primitiveDart != null) {
        ...
        buf.writeln("  static const Color $dartName = PregoColorPrimitives.$primitiveDart;");
      } else {
...
void _writeSemanticColorClass(
  ...
  buf.writeln("final class PregoColors {");
  ...
  buf.writeln("  // ===========================================================================");
  buf.writeln();
  buf.writeln("  static const dark = PregoColors._(");
  buf.writeln("    brightness: Brightness.dark,");
  _writeConstructorArgs(buf, colors, "PregoColorsDark");
  buf.writeln("  );");

  // --- const constructor ---
  buf.writeln();
  buf.writeln("  const PregoColors._({");
  ...
  buf.writeln("  // ===========================================================================");
  buf.writeln();
  buf.writeln("  static const light = PregoColors._(");
  buf.writeln("    brightness: Brightness.light,");
  _writeConstructorArgs(buf, colors, "PregoColorsLight");
  buf.writeln("  );");
...
String _primitiveToDartName(String group, String shade) {
  ...
  final normalized = group.replaceAll("(", "").replaceAll(")", "").trim();
...
void _generateSpacingPrimitivesFile(List<Map<String, dynamic>> spacingPrimitives) {
  ...
  buf.writeln("/// Semantic tokens in [PregoSpacing] and [PregoWidths] reference these constants.");
  buf.writeln("///");
  buf.writeln("/// Do not use primitives directly in widgets — use semantic tokens via");
  buf.writeln("/// `context.prego.spacing.*` instead.");
  buf.writeln("abstract final class PregoSpacingPrimitives {");
...
void _generateSpacingFile(
  ...
  buf.writeln("//   dart run scripts/figma_tokens/sync_figma_tokens.dart generate");
  buf.writeln();
  buf.writeln('import "prego_spacing_primitives.g.dart";');
  buf.writeln();
  buf.writeln("/// Semantic spacing tokens matching Figma spacing scale.");
  buf.writeln("///");
  buf.writeln("/// Values reference [PregoSpacingPrimitives] where Figma uses an alias.");
  buf.writeln("///");
  buf.writeln("/// Usage:");
  buf.writeln("/// ```dart");
  buf.writeln("/// Padding(padding: EdgeInsetsDirectional.all(PregoSpacing.md))");
  buf.writeln("/// SizedBox(height: PregoSpacing.lg)");
  buf.writeln("/// ```");
  buf.writeln("abstract final class PregoSpacing {");
...
void _generateRadiusFile(List<Map<String, dynamic>> radiusTokens) {
  ...
  buf.writeln("///     borderRadius: BorderRadius.circular(PregoRadius.md),");
  buf.writeln("///   ),");
  buf.writeln("/// )");
  buf.writeln("/// ```");
  buf.writeln("abstract final class PregoRadius {");
...
void _generateWidthsFile(
  ...
  buf.writeln("///");
  buf.writeln("/// Values reference [PregoSpacingPrimitives] where Figma uses an alias.");
  buf.writeln("abstract final class PregoWidths {");
...
void _writeFloatConstant(
  ...
  if (alias != null) {
    ...
    if (primitiveDart != null) {
      ...
      buf.writeln("  static const double $dartName = PregoSpacingPrimitives.$primitiveDart;");
      return;
    }
  }
  buf.writeln("  /// ${_formatDouble(value)}px - Figma: $figmaName");
