...
void main() {
  group("SessionService", () {
    ...
    group("getMessageDiffs", () {
      test("returns list of FileDiff on success", () async {
        final fileDiffs = [
          const FileDiff(
            file: "lib/utils.dart",
            before: "String format(String s) => s;",
            after: "String format(String s) => s.toUpperCase();",
            additions: 1,
            deletions: 1,
            status: FileDiffStatus.modified,
          ),
        ...
        when(
          () => mockClient.get<List<FileDiff>>(
            "/session/session-123/message/msg-456/diff",
            fromJson: any(named: "fromJson"),
          ),
        ).thenAnswer((_) async => ApiResponse.success(fileDiffs));

        final result = await sessionService.getMessageDiffs("session-123", "msg-456");

        expect(result, isA<SuccessResponse<List<FileDiff>>>());
        final data = (result as SuccessResponse<List<FileDiff>>).data;
        expect(data, equals(fileDiffs));
        expect(data.length, equals(1));
        expect(data[0].file, equals("lib/utils.dart"));
        expect(data[0].additions, equals(1));
        expect(data[0].deletions, equals(1));
      });

      test("returns empty list when message has no diffs", () async {
        when(
          () => mockClient.get<List<FileDiff>>(
            "/session/session-123/message/msg-789/diff",
            fromJson: any(named: "fromJson"),
          ),
        ).thenAnswer((_) async => ApiResponse.success(<FileDiff>[]));

        final result = await sessionService.getMessageDiffs("session-123", "msg-789");
      ...
      test("returns error response on failure", () async {
        ...
        when(
          () => mockClient.get<List<FileDiff>>(
            "/session/session-999/message/msg-000/diff",
            fromJson: any(named: "fromJson"),
          ),
        ).thenAnswer((_) async => ApiResponse.error(error));

        final result = await sessionService.getMessageDiffs("session-999", "msg-000");
      ...
      test("constructs correct endpoint URL with sessionId and messageId", () async {
        when(
          () => mockClient.get<List<FileDiff>>(
            "/session/my-session/message/my-message/diff",
            fromJson: any(named: "fromJson"),
          ),
        ).thenAnswer((_) async => ApiResponse.success(<FileDiff>[]));

        await sessionService.getMessageDiffs("my-session", "my-message");
