...
| Mutation boundary | No draft mutation until terminal outcome: realtime preview must not write `_draft`, `TextEditingController`, or `onDraftChanged` while events are streaming. On terminal success, append only confirmed text through the existing voice-transcript draft path; on cancel or failed terminal outcome, clear preview and never commit provisional text. |
