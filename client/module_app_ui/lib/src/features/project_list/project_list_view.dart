...
typedef ProjectListDisconnectedViewBuilder = Widget Function({
  required BuildContext context,
  required ProjectListBridgeDisconnected state,
  required BridgeSummary? bridge,
});

typedef ProjectListDisconnectedActionBuilder = Widget? Function({
  required BuildContext context,
  required ProjectListBridgeDisconnected state,
});

typedef ProjectListConnectedEmptyViewBuilder = Widget Function({required BuildContext context});
...
class const ProjectListView({
  ...
  required final ProjectListConnectionBannerBuilder connectionBannerBuilder,
  required final ProjectListDisconnectedRefresh onRefreshDisconnected,
...
class _ProjectListViewState() extends State<ProjectListView> {
  ...
  /// connected-but-empty state carries its own injected add-project call to
  /// action, which a floating pill would only duplicate.
  ({Widget? action, PregoFloatingActionAlignment alignment}) _floatingAction({
    ...
    if (state is ProjectListBridgeDisconnected) {
      return (
        action: widget.disconnectedActionBuilder(context: context, state: state),
    ...
    return PregoGlassScaffold(
      ...
      actions: [
        PregoButtonsIconGlass(
          ...
          onPressed: () => widget.onOpenSettings(context: context),
        ),
      ],
  ...
  /// nothing to pull for.
  ///
  /// A loaded list re-fetches its projects. Product shells decide what a pull
  /// means while disconnected: mobile retries relay connection, while desktop
  /// requests the supervised bridge to start.
  Future<void> Function()? _refreshFor({required BuildContext context, required ProjectListState state}) {
    return switch (state) {
      ...
      final ProjectListBridgeDisconnected disconnected => () => widget.onRefreshDisconnected(
        context: context,
        state: disconnected,
      ),
  ...
  Widget? _bannerFor({required BuildContext context, required ProjectListState state}) {
    ...
    final banner = widget.connectionBannerBuilder(context: context);
    if (state.projects.isNotEmpty) return banner;
    // The connection state is already watched above, so a read here still
  ...
  List<Widget> _buildContentSlivers({
    ...
    return switch (state) {
      ...
      final ProjectListBridgeDisconnected disconnected => [
        SliverFillRemaining(
          hasScrollBody: false,
          child: SafeArea(
            top: false,
            child: widget.disconnectedViewBuilder(
              context: context,
              state: disconnected,
              bridge: bridge,
            ),
