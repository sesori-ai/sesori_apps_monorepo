...
enum LegacySessionOptionSource() {
  agents,
  providers,
  commands,
}

final class const LegacySessionOptionError({
  required final LegacySessionOptionSource source,
  required final ApiError error,
});

final class const LegacySessionOptionsRepositoryAvailable({required final SessionOptionsCatalog catalog})
    extends LegacySessionOptionsRepositoryResult;

final class LegacySessionOptionsRepositoryFailure({required List<LegacySessionOptionError> errors})
    extends LegacySessionOptionsRepositoryResult {
  final List<LegacySessionOptionError> errors = List.unmodifiable(errors);
}

final class LegacySessionOptionsRepositoryPartial({
  required final SessionOptionsCatalog catalog,
  required List<LegacySessionOptionError> errors,
}) extends LegacySessionOptionsRepositoryResult {
  final List<LegacySessionOptionError> errors = List.unmodifiable(errors);
}
...
final class const SessionOptionsRepositoryAvailable({
  required final SessionOptionsCatalog catalog,
  required final bool isStale,
}) extends SessionOptionsRepositoryResult;
...
final class const SessionOptionsRepositoryProjectNotFound({required final ApiError error})
    extends SessionOptionsRepositoryResult;

final class const SessionOptionsRepositoryAuthenticationRequired({required final String actionHint})
    extends SessionOptionsRepositoryResult;
...
final class const SessionOptionsRepositoryFailure({required final ApiError error})
    extends SessionOptionsRepositoryResult;
