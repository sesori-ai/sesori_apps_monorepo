...
sealed class BridgeIdentityState with _$BridgeIdentityState {
  ...
  const factory BridgeIdentityState.pending() = BridgeIdentityPending;

  /// [bridge] is the machine to name — the account's most recently seen
  /// registered bridge.
  const factory BridgeIdentityState.named({required BridgeSummary bridge}) = BridgeIdentityNamed;
  ...
  const factory BridgeIdentityState.unnamed() = BridgeIdentityUnnamed;
}
