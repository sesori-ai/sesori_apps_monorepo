...
class SessionDetailCubit(
  ...
  void _onManagementResult(PluginManagementLoadResult result) {
    ...
    switch (current) {
      ...
      case SessionDetailLoading() || SessionDetailHarnessUnavailable() || SessionDetailFailed():
        break;
    }
  }
      ...
      case SessionDetailMetadataFound(:final session):
        switch (result) {
          ...
          case SessionDetailLoadResultLoaded(:final snapshot):
            ...
            if (state case SessionDetailLoaded(deviceCanvas: DeviceCanvasSessionLoading())) {
      ...
      if (current is SessionDetailLoaded) {
        ...
        emit(
          current.copyWith(
            ...
            deviceCanvas: _deviceCanvasService == null ? current.deviceCanvas : const DeviceCanvasSessionDisconnected(),
          ),
        );
      }
      return;
      ...
      switch (result) {
        case DeviceCanvasStatusSupported(:final status):
          ...
          final mutation = switch (previousMutation) {
            ...
            DeviceCanvasSessionMutationFailed() ||
            null => const DeviceCanvasSessionMutationIdle(),
          };
          emit(
            current.copyWith(
  ...
  bool _isDeviceCanvasMutationCommitted({
    ...
    return switch (action) {
      DeviceCanvasSessionMutationAction.claim ||
      DeviceCanvasSessionMutationAction.reassign => claim?.sessionId == _sessionId,
      DeviceCanvasSessionMutationAction.release =>
  ...
  bool _hasUncertainDeviceCanvasMutation(DeviceCanvasSessionState state) {
    return switch (state) {
      ...
      DeviceCanvasSessionReady() => false,
    };
  }
