...
class MacosAuthenticatedUpgradeTests(unittest.TestCase):
    ...
    def test_native_keychain_writer_reads_secret_only_from_stdin(self):
        ...
        self.assertIn("kSecAttrSynchronizable: false", query_source)
        self.assertIn("kSecAttrAccessible: kSecAttrAccessibleWhenUnlocked", query_source)
        self.assertIn("kSecReturnData: true", query_source)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                json.loads(recorded),
                {
                    ...
                    "helperPid": 501,
                    "helperObservedDuringWait": True,
                    "bridgeLogActivityObserved": True,
    ...
    def test_helper_timeout_does_not_copy_raw_bridge_output(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            recorded = (root / "current-authenticated-helper.json").read_text(encoding="utf-8")
            self.assertNotIn("private-token-value", recorded)
            observation = json.loads(recorded)
    ...
    def test_private_app_startup_diagnostics_expose_only_closed_markers(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertNotIn("private-token-value", recorded)
            self.assertNotIn("/private/user/path", recorded)
            self.assertEqual(
                json.loads(recorded),
                {
                    "appLogPresent": True,
