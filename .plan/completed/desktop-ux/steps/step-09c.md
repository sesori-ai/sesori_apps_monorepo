...
## Initial reviewed plan and implementation
...
- The initial `DesktopSidebarRefreshOrchestrator` factory used the cockpit's same two services via required params.
  Its placement was corrected to Layer-3 `DesktopSidebarRefreshService` after the review finding recorded below.
...
## Operator-approved Layer-3 correction

Codex identified that the new Cubit imported another Layer-4 class, violating the scoped no-same-layer rule despite
both earlier implementation approvals. Existing logout/takeover examples did not override that rule. After the
review limit was reached, the operator explicitly selected moving only this workflow to Layer 3 and authorized one
further focused architecture check. The earlier placement approvals are superseded, not evidence of an exception.

The workflow is now `DesktopSidebarRefreshService` in `services/desktop_sidebar_refresh_service.dart`.
Its body, outcome enum and two scoped factory parameters are unchanged. The Cubit imports the Layer-3 service;
composition, exports, mocks and test paths update together, without a compatibility alias. Desktop-core DI was
regenerated by its owning builder. No pre-existing orchestrator was moved and no inventory, owner or policy changed.
Plan: `/tmp/rose-elephant-1545-layer3-correction-plan.md`. Current regression guidance names the service; historical
commands above retain the filenames actually used at those earlier checkpoints.

The following runs used an **uncommitted tree based on `9a34103423c1e117bbc6db5ac45f3e600e63d553`**, not a later
source commit. All exited 0: nine service/Cubit cases, 41 cockpit/provider cases, both owning analyzers and generation.
These 50 cases overlap preceding evidence; do not add the totals. The original core behavior and rendered layout
were unchanged, so core suites and previews were not rerun. Receipts and saved-patch hashes:
`/tmp/rose-elephant-1545-layer3-verification.json` and `/tmp/rose-elephant-1545-layer3-generation.json`.
...
(cd client/desktop && "$SDK/flutter" test --no-pub --reporter=json \
  test/core/widgets/desktop_cockpit_cubit_provider_test.dart test/core/widgets/desktop_cockpit_shell_test.dart)
(cd client/desktop && "$SDK/dart" analyze --fatal-infos)
# Formatting: eight files, one changed; exit 0 before the test checkpoint.
"$SDK/dart" format client/module_desktop_core/lib/src/services/desktop_sidebar_refresh_service.dart \
  client/module_desktop_core/lib/src/cubits/desktop_sidebar/desktop_sidebar_refresh_cubit.dart \
  client/module_desktop_core/lib/sesori_desktop_core.dart \
  client/module_desktop_core/test/services/desktop_sidebar_refresh_service_test.dart \
  client/module_desktop_core/test/cubits/desktop_sidebar/desktop_sidebar_refresh_cubit_test.dart \
