...
| E2E-11 | Models | Switched mid-session from default to `cx/gpt-5.4-mini` and sent a turn. | This authenticated CLI's live configured catalog returned the exact `cx/*` IDs recorded here. The turn returned `MODEL OK`, and the navbar changed to `claude - gpt-5.4-mini`. A later default-model turn restored `gpt-5.6-terra`. |
