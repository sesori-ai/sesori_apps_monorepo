...
`desktop-qualification.yml` offers `native-builds` for the six-platform matrix plus
closed macOS modes: `macos-signing-preflight`, `macos-packaging`, `macos-gui-probe`,
and the later credential-free `macos-upgrade-probe` continuation.
