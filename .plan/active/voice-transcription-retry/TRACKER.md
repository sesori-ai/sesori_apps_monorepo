...
- [x] User approved one cohesive Step 3 after the candidate measured roughly 3,300 touched lines including about 977 causal legacy deletions; the final 2,800-4,100 budget includes architecture/PR-review lifecycle fixes without introducing a temporary duplicate lifecycle or compatibility wrapper.
...
| [ ] | 3/5 | apps | `🚧 [voice-transcription-retry] Move voice lifecycle into client core [step 3/5]` | 2,800-4,100 | [PR #1162](https://github.com/sesori-ai/sesori_apps_monorepo/pull/1162) open; 4,065 changed lines, architecture approved, seven review findings addressed, final verification passing |
...
- **Step 3 ownership migration:** PR [#1162](https://github.com/sesori-ai/sesori_apps_monorepo/pull/1162), 4,065 changed lines; synchronized with apps `origin/main` at `746e222c42`; codegen and module_core/app strict analysis pass, along with 25 focused core tests, 15 platform/path tests, 86 composer tests, and 59 new-session/routing tests; architecture approved, then seven PR review findings were addressed across typed causes, stop/cancel disposal serialization, both conditional composer lifetimes, native prewarm coordination, and concurrent path uniqueness
