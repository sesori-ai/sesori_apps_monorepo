...
- Automation never opens a turn and is never a sticky prompt. Only a turn's
  opener is sticky.
- One global fold state. No per-turn or persisted fold state. Every control
  switches it through the body's single setter.
...
- Step 6 does not close until pinch has been checked on a real iPhone and on a
  real macOS trackpad.
...
| 4 | `turn-navigation/fold-turns` | [4](#fixed-pr-titles) | ≤ 900 | 2 |
| 5 | `turn-navigation/keep-place` | [5](#fixed-pr-titles) | ≤ 500 | 4 |
| 6 | `turn-navigation/pinch` | [6](#fixed-pr-titles) | ≤ 600 | 5 |
| 7 | `turn-navigation/sticky-prompt` | [7](#fixed-pr-titles) | ≤ 600 | 5 |
| 8 | `turn-navigation/desktop-index` | [8](#fixed-pr-titles) | ≤ 800 | 7 |
| 9 | `turn-navigation/regression-docs` | [9](#fixed-pr-titles) | ≤ 300 | 2–8 |
| 10 | `turn-navigation/retire` | [10](#fixed-pr-titles) | ≤ 200 | 9 |

Step 3's target includes regenerated DTO output. Step 4's includes generated
localization files.
...
4. `⚙️ [turn-navigation] Fold every turn into one line with a bar toggle [step 4/10]`
5. `⚙️ [turn-navigation] Keep the reader's turn in place when turns fold or unfold [step 5/10]`
6. `⚙️ [turn-navigation] Pinch to fold and unfold turns on touch and trackpad [step 6/10]`
7. `⚙️ [turn-navigation] Pin the current turn's prompt while reading it [step 7/10]`
8. `⚙️ [turn-navigation] Add the desktop turn index [step 8/10]`
9. `🌱 [turn-navigation] Reconcile the turn navigation regression docs [step 9/10]`
10. `🌱 [turn-navigation] Record the turn navigation matrix and retire the plan [step 10/10]`
