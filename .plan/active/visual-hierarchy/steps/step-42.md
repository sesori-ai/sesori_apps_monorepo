...
- While the session works and no step is live, a "Working…" live row with the
  same sparkle closes the transcript: before the first token and between steps.
  A starting step takes its place; the working row eases its height and fades
...
- `module_app_ui`: `test/features/session_detail` and `test/widgets`, all
  ...
  when the step finishes, and it leaves when idle or when a retry row shows.
