...
- A 44-square opaque tap area around it keeps the touch target, so a tap just
  outside the drawn button still opens the parent folder. The tap area is
  excluded from semantics; the button's own "Parent folder" semantics stay.
- The pointer size (`sm`, 36) is unchanged.

## Verification

- `add_project_dialog_test.dart` passes (25). A new case checks the touch
  button is 40 by 40 and a tap 1 px inside the 44 target still goes up.
