...
sealed class const DeviceCanvasRtcDescription._() with _$DeviceCanvasRtcDescription {
  const factory({
    @JsonKey(unknownEnumValue: DeviceCanvasRtcDescriptionType.unknown) required DeviceCanvasRtcDescriptionType type,
    required String sdp,
...
sealed class const DeviceCanvasStreamStartResponse._() with _$DeviceCanvasStreamStartResponse {
  const factory({
    @JsonKey(unknownEnumValue: DeviceCanvasStreamStartOutcome.unknown) required DeviceCanvasStreamStartOutcome outcome,
    required String? leaseId,
    required int? expiresAt,
...
sealed class const DeviceCanvasStreamStatusResponse._() with _$DeviceCanvasStreamStatusResponse {
  ...
  bool get isValid => switch (outcome) {
    DeviceCanvasStreamStatusOutcome.active =>
      _isValidDeviceCanvasStreamPayload(
            leaseId: leaseId,
            expiresAt: expiresAt,
            answer: answer,
            iceCandidates: iceCandidates,
...
bool _isDeviceCanvasIpv4EmbeddedIpv6(List<int> segments) {
  ...
  final translatable = segments.take(4).every((segment) => segment == 0) && segments[4] == 0xffff && segments[5] == 0;
