...
class OpenCodeRuntimeAsset {
  const OpenCodeRuntimeAsset({
...
/// Pinned facts about the OpenCode runtime the bridge can install and gate.
///
/// Two version constants drive provisioning:
/// - [minSupportedVersion] gates a *pre-installed* (PATH) OpenCode: at or above
///   it, the bridge uses the user's own install; below it, the bridge falls back
///   to the managed runtime (so a too-old install can't break the bridge, and a
...
/// 4. Raise [minSupportedVersion] only if the new bridge code requires a newer
///    OpenCode API than older installs provide.
class OpenCodeRuntimeManifest {
  ...
  static final SemanticVersion minSupportedVersion = SemanticVersion.parse(value: "1.0.0");

  /// The exact OpenCode version the managed runtime installs.
  static final SemanticVersion bundledVersion = SemanticVersion.parse(value: "1.17.9");
  ...
  /// linux ships `.tar.gz`; the non-baseline, non-musl CLI builds are used.
  static const Map<PlatformOs, Map<PlatformArch, OpenCodeRuntimeAsset>> _assets = {
    PlatformOs.macos: {
      PlatformArch.arm64: OpenCodeRuntimeAsset(
        assetName: "opencode-darwin-arm64.zip",
        format: ArchiveFormat.zip,
        sha256: "913d813a88ca4f6209b9c48e548bd376eef4d1e74c2bb113aa91aa96c784d332",
      ),
      PlatformArch.x64: OpenCodeRuntimeAsset(
  ...
  OpenCodeRuntimeAsset? assetFor({required PlatformTarget target}) {
    return _assets[target.os]?[target.arch];
  }

  /// The download URL for [asset] at [bundledVersion].
  String downloadUrlFor({required OpenCodeRuntimeAsset asset}) {
