...
/// awaitable [initialize] so the descriptor can complete cold-start (opening the
/// `codex app-server` WebSocket and performing the `initialize` handshake) — and
/// surface a failure as a degraded status — before returning. Keeping this as a
/// small interface, rather than the concrete `CodexPlugin`, lets the
/// descriptor's API construction be a test seam without forcing a real socket in
/// unit tests.
abstract interface class CodexManagedApi implements BridgePluginApi {
