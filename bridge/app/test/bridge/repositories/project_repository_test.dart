...
void main() {
  group("ProjectRepository", () {
    ...
    test("native activity reuses an existing path-canonical project row", () async {
      ...
      expect(observation!.evidence.single.projectId, directory);
    ...
    test("native activity atomically resolves identity against catalog import", () async {
      ...
      final observation = await reconciliation;
      await catalogImport;

      final rows = await db.projectsDao.getAllProjects();
      expect(rows, hasLength(1));
      expect(observation!.evidence.single.projectId, rows.single.projectId);
    });
    ...
    test("activity publication rejects evidence from a replaced generation", () async {
      await db.projectsDao.setActivity(
      ...
      await expectLater(
        generationAwareActivityRepo.commitActivities(
          observation: (pluginId: plugin.id, generation: 1, evidence: const []),
          activities: const {
            "stable-project": (createdAt: 1, updatedAt: 99),
          },
        ),
        throwsA(isA<PluginOperationException>()),
      );

      final stored = await db.projectsDao.getProject(projectId: "stable-project");
      expect(stored?.updatedAt, 2);
    });
