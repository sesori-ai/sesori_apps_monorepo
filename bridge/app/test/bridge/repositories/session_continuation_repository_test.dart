...
void main() {
  ...
  void openDatabase() {
    ...
    repository = SessionContinuationRepository(dao: SessionContinuationDao(database: db), runtime: runtime);
  }
  ...
  setUp(() async {
    ...
    await insertTestSession(
      db: db, sessionId: "session", backendSessionId: "backend", pluginId: "plugin", projectId: "/fixture",
      isDedicated: false, createdAt: 1, worktreePath: null, branchName: null, baseBranch: null, baseCommit: null,
      agent: null, agentModel: null,
    );
  });
  tearDown(() async {
    await db.close();
  ...
  Future<bool> observe({required int generation, required String errorId}) =>
      repository.recordObservationForCurrentGenerationAlreadyReserved(
        sessionId: "session", pluginId: "plugin", generation: generation,
        errorMessageId: errorId, observedAt: observedAt, resetAt: resetAt,
      );
  ...
  test("preference is independent of observations and pause deadlines survive database reopen", () async {
    ...
    expect(await repository.readReadyToCheck(sessionId: "session", resetCutoff: observedAt, pausedRecheckCutoff: resetAt), isNull);
    final record = await repository.readReadyToCheck(sessionId: "session", resetCutoff: resetAt, pausedRecheckCutoff: resetAt);
    final recheckAt = resetAt.add(const Duration(minutes: 7));
    final paused = SessionContinuationOutcome.paused(errorMessageId: "error", observedAt: observedAt,
      resetAt: resetAt, reason: AutoContinuationPauseReason.busy, recheckAt: recheckAt);
    ...
    expect(await repository.readReadyToCheck(sessionId: "session", resetCutoff: recheckAt, pausedRecheckCutoff: resetAt), isNull);
  ...
  test("cancelled and consumed observations cannot rearm after a restart; later errors retain opt-in", () async {
    ...
    final consumed = SessionContinuationOutcome.consumed(errorMessageId: "second-error", promptId: "prompt", attemptedAt: resetAt);
  ...
  test("generation fence rejects stale observations and cancellation before persistence", () async {
    ...
    await expectLater(repository.cancelForCurrentGenerationAlreadyReserved(sessionId: "session", pluginId: "plugin", generation: 2), throwsA(isA<PluginOperationException>()));
    expect((await repository.read(sessionId: "session")).outcome.observationId, "current");
    expect(await repository.cancelForCurrentGenerationAlreadyReserved(sessionId: "session", pluginId: "plugin", generation: 1), isTrue);
  });
  ...
  test("all persisted outcomes round-trip and session deletion cascades storage", () async {
    final outcomes = [
      ...
      const SessionContinuationOutcome.submissionFailed(errorMessageId: "failed", reason: AutoContinuationFailureReason.submissionRejected),
    ];
    for (final outcome in outcomes) {
      await repository.writeOutcomeAlreadyReserved(record: await repository.read(sessionId: "session"), outcome: outcome);
