...
void main() {
  ...
  test("serializes projects and coalesces duplicate scheduling", () async {
    ...
    await service.schedule(projectId: "project-a");
    await firstStarted.future;
    await service.schedule(projectId: "project-a");
    await service.schedule(projectId: "project-b");
  ...
  test("fills only the remaining capacity and ignores existing case variants", () async {
    ...
    await service.schedule(projectId: projectId);
    await service.drain();
  ...
  test("rejects population when bridge-local key material cannot be loaded", () async {
    final glossaryRepository = _FakeProjectGlossaryRepository(
      existingWords: const {},
      source: null,
      getWordsCallback: null,
    ...
    expect(await service.schedule(projectId: "project"), isNull);
    await service.drain();
  ...
  test("a failed best-effort attempt does not retry during the same bridge process", () async {
    ...
    await service.schedule(projectId: "project");
    await service.schedule(projectId: "project");
    await service.drain();
