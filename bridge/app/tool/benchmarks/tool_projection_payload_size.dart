...
void main() {
  ...
  stdout.writeln("Typical uses already-bounded normalized text; stress deliberately bypasses plugin-local bounds.");
  stdout.writeln("sample,scenario,unfiltered,before,after");
  for (final sample in [
    (name: "typical-bounded", output: "Synthetic command output\n" * 20, error: "Synthetic failure detail\n" * 16),
    (name: "oversized-stress", output: "😀" * 12000, error: "é" * 12000),
  ]) {
    for (final fixture in [
      ...
      stdout.writeln(
        "${sample.name},${fixture.name},${bytes(state: raw)},${bytes(state: before)},${bytes(state: after)}",
      );
    }
  }
}
