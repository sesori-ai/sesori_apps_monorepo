...
/// Warms currently viewed sessions when session-open warm-up becomes enabled.
class PluginWarmupSettingListener({
  ...
  void start() {
    ...
    _settingsService.states
        .listen(
          (enabled) => _handleSetting(enabled: enabled),
          onError: (Object error, StackTrace stackTrace) {
            if (_disposed) return;
            Log.w("Plugin warm-up settings tracking failed unexpectedly", error, stackTrace);
          },
        )
        .addTo(_subscriptions);
  }

  void _handleSetting({required bool enabled}) {
    if (_disposed || !enabled) return;
    for (final sessionId in _tracker.activeSessionIds) {
      unawaited(
  ...
  Future<void> _warm({required String sessionId}) async {
    try {
      ...
      if (_disposed) return;
      Log.w('Setting-enabled plugin warm-up failed for session "$sessionId"', error, stackTrace);
    }
  }
