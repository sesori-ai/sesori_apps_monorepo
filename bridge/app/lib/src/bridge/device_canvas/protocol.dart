...
sealed class DeviceCanvasOutboundMessage with _$DeviceCanvasOutboundMessage {
  ...
  const factory helloAccepted({required int protocolVersion, required String bridgeId}) = DeviceCanvasHelloAccepted;

  @FreezedUnionValue("claimsSnapshot")
  const factory claimsSnapshot({required List<DeviceCanvasClaimProjectionDto> claims}) = DeviceCanvasClaimsSnapshot;

  @FreezedUnionValue("claimUpdated")
  const factory claimUpdated({required DeviceCanvasClaimProjectionDto claim}) = DeviceCanvasClaimUpdatedMessage;
