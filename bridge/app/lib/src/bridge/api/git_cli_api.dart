...
class GitCliApi {
  ...
  Future<ProcessResult> fetchRemotes({required String workingDirectory}) {
    return _processRunner.run("git", ["fetch", "--all"], workingDirectory: workingDirectory);
  }

  Future<ProcessResult> listBranches({required String workingDirectory}) {
    return _processRunner.run(
      "git",
      ["branch", "-a", "--format=%(refname:short) %(creatordate:unix)"],
      workingDirectory: workingDirectory,
    );
  }

  Future<ProcessResult> listWorktrees({required String workingDirectory}) {
    return _processRunner.run("git", ["worktree", "list", "--porcelain"], workingDirectory: workingDirectory);
  }

  Future<ProcessResult> getCurrentBranch({required String workingDirectory}) {
    return _processRunner.run("git", ["rev-parse", "--abbrev-ref", "HEAD"], workingDirectory: workingDirectory);
  }
