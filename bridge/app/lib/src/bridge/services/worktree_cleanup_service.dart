...
Future<CleanupResult> performWorktreeCleanup({
  ...
  if (deleteWorktree) {
    ...
    if (!removed) {
      Log.w("worktree cleanup: failed to remove worktree for session $sessionId at $worktreePath");
    }
  }
  ...
  if (deleteBranch) {
    ...
    if (!deleted) {
      Log.w("worktree cleanup: failed to delete branch $branchName for session $sessionId");
    }
  }
