...
class SessionUnseenService {
  ...
  /// activity so it is immediately unseen.
  ...
  Future<void> markRead({required String sessionId}) async {
    final row = await _unseenRepository.getUnseenRow(sessionId: sessionId);
    if (row == null) return;
  ...
  /// "Mark as Unread": clear seen so it bolds iff activity is newer than the
  /// user's last message.
  Future<void> markUnread({required String sessionId}) async {
    final row = await _unseenRepository.getUnseenRow(sessionId: sessionId);
    if (row == null) return;
    await _unseenRepository.markSessionUnseen(sessionId: sessionId);
    await _emit(sessionId: sessionId, projectId: row.projectId);
  }

  Future<void> _onViewStarted(String sessionId) async {
    final row = await _unseenRepository.getUnseenRow(sessionId: sessionId);
    if (row == null) return;
