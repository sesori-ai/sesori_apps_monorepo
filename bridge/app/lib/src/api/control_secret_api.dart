...
class ControlSecretApi({required final Stream<List<int>> _input}) {
  ...
  Future<String> readSecret({Duration timeout = const Duration(seconds: 10)}) async {
    final firstLine = await _input.transform(utf8.decoder).transform(const LineSplitter()).first.timeout(timeout);
